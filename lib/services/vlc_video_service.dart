...
class VLCVideoService {
  ...
  void _setupEventListeners(VlcPlayerController controller, Function(String)? onError) {
    ...
    controller.addOnInitListener(() {
      debugPrint('VLC Player inicializado correctamente');
    });
    ...
    controller.addListener(() {
      ...
      if (value.hasError) {
        debugPrint('Error en VLC Player: ${value.errorDescription}');
        onError?.call(_getErrorMessage(value.errorDescription));
      }
      
      debugPrint('Estado VLC - Reproduciendo: ${value.isPlaying}, Duración: ${value.duration}, Posición: ${value.position}');
    });
  }
  ...
  void _logStreamError(String url, String error) {
    ...
    debugPrint('[$timestamp] ERROR VLC STREAM: $url - $error');
  }
  ...
  void setAspectRatio(String aspectRatio) {
    ...
    debugPrint('Configurando aspecto de pantalla: $aspectRatio');
  }
